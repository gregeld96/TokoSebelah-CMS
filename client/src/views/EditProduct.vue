<template>
  <div>
        ...
        <form @submit.prevent="editProduct" class="my-2 universal-form mx-auto">
                <div class="form-group">
                    <label><h4>Product Name</h4></label>
                    <input type="text" class="form-control w-50 mx-auto" v-model="name" id="title-add" />
                </div>
                <div class="form-group">
                    <label><h4>Image Url</h4></label>
                    <input type="text" class="form-control w-50 mx-auto" v-model="image_url" id="description-add" />
                </div>
...
export default {
  ...
  created () {
    this.name = this.selectProduct.name
    this.image_url = this.selectProduct.image_url
    this.price = this.selectProduct.price
    this.stock = this.selectProduct.stock
    this.categoryId = this.selectProduct.categoryId
  }
}
</script>
